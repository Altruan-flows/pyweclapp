...
class DocManager:
    ...
    @staticmethod
    def convertPdfToTiff(
        docs: List[io.BytesIO], names: List[str]
    ) -> Tuple[List[io.BytesIO], List[str]]:
        try:
            import fitz
        except ImportError:
            logging.error(
                "Special Functionality requires fitz module to manually install (pip install PyMuPDF)"
            )
    ...
    def downloadDocById(docId) -> io.BytesIO:
        content = weclapp.GET(
            entity_name="document", entity_id=f"{docId}/download", as_type=bytes
        )
        file = io.BytesIO()
        file.write(content)
        file.seek(0)
    ...
    def getDocuments(self) -> List[Document]:
        ...
        response = weclapp.GET(
            ...
            query={"entityName": self.entityName, "entityId": self.entityId},
        )
        for doc in response:
    ...
    def setDescriptionOfLatestDocument(self, docType: config.ALLOWED_DOC_TYPES_LITERAL):
        ...
        docsWithoutDescription = [
            doc for doc in self.getDocuments() if not doc.description
        ]
    ...
    def queryDoc(
        self, value: str, key: str = "docType", raiseError: bool = True
    ) -> Document:
        ...
        for doc in self.documents:
            if key in [
                ...
                if (
                    hasattr(doc.description, key)
                    and getattr(doc.description, key) == value
                ):
                    return doc
        if raiseError:
    ...
    def uploadFile(
        ...
        name, docType = Document.documentNamingConvention(
            docType=docType, docFormat=docFormat, fullName=name
        )
        ...
        try:
            response = weclapp.POST(
                entityName="document/upload",
                query={
            ...
            if e.isOptimisticLock:
                logging.error(
                    f"Optimistic Locking Error while uploading File {name}... retrying"
                )
                time.sleep(1)
                response = weclapp.POST(
                    entityName="document/upload",
    ...
    def getDocumentFiles(
        self,
        by: Literal["all", "latest"],
        docType: str = config.ALLOWED_DOC_TYPES_LITERAL,
        docFormat: str = config.ALLOWED_DOC_FORMATS,
    ) -> Tuple[List[io.BytesIO], List[str]]:
        ...
        _, docType = Document.documentNamingConvention(
            docType=docType, docFormat=docFormat, fullName=""
        )
        ...
        for doc in self.documents:
            ...
            if (
                docFormat in doc.name
                and doc.description
                and doc.description.docType == docType
            ):
