...
def parse(
    time: Union[
        time.time, datetime.date, datetime.datetime, str, int, float
    ] = time.time()
    * 1000
) -> datetime.datetime:
    try:
        ...
        if type(time) is datetime.date:
            relevantTime = datetime.datetime(
                year=time.year, month=time.month, day=time.day
            )
            logging.warning("date found")
...
def add(
    startTime: Union[time.time, datetime.date, datetime.datetime, str, int, float],
    years=0,
    months=0,
    days=0,
    leapdays=0,
    ...
    """Parses a localized datetimeObject from a string or timestamp and adds
    the given time to it (timezone shifts aware)
    ...
    try:
        ...
        raise ValueError(
            f"---util.convertDateTimeTo()--- -> >>{startTime}<< not a datetimeobject"
        )

    newDate = startTime + relativedelta(
        years=years,
        months=months,
        days=days,
        leapdays=leapdays,
